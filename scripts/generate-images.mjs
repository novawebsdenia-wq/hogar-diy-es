...
function extractAffiliateCards(content) {
  ...
  for (const match of content.matchAll(/<AffiliateCard[\s\S]*?\/>/gm)) {
    ...
    if (nameMatch) {
      cards.push({ idx: ++idx, name: nameMatch[1], fullMatch: match[0], offset: match.index })
    }
  }
  return cards
}
...
async function processArticle(category, slug, filePath) {
  ...
  for (const card of cardsToProcess) {
    ...
    if (!existsSync(toolPath)) {
      try {
        process.stdout.write(`  🔧 Herramienta ${card.idx}: ${card.name.slice(0, 40)}...`)
  ...
  const existingSteps = steps.filter((_, i) => existsSync(join(imageDir, `paso-${i + 1}.jpg`)))
  if (existingSteps.length > 0) {
    updated = insertStepImages(updated, steps, category, slug)
  }

  // Insertar imágenes de herramientas
  const existingCards = cardsToProcess.filter(c => existsSync(join(imageDir, `herramienta-${c.idx}.jpg`)))
...
async function main() {
  ...
  const categories = readdirSync(CONTENT_DIR).filter(
    c => !c.startsWith('.')
  )
