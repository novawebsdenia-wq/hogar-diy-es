...
function getAllArticles() {
  ...
  categories.forEach(category => {
    const files = fs.readdirSync(path.join(contentDir, category)).filter(f => f.endsWith('.mdx'));
    files.forEach(file => {
...
articles.forEach((article, index) => {
  ...
  if (paragraphs.length > 4 && link1) {
    paragraphs.splice(quarterPoint, 0, `> 📌 **Lectura recomendada:** [${link1.title}](/${link1.category}/${link1.slug})`);
    linksAdded++;
  }

  // Inyectar enlace 2 en la mitad/final
  if (paragraphs.length > 8 && link2) {
    paragraphs.splice(midPoint + 1, 0, `> 📌 **Lectura relacionada:** [${link2.title}](/${link2.category}/${link2.slug})`);
    linksAdded++;
  } else if (paragraphs.length <= 8 && link1 && link2) {
    // Si es corto, meter el segundo casi al final
    paragraphs.splice(paragraphs.length - 1, 0, `> 📌 **Guía paso a paso:** [${link2.title}](/${link2.category}/${link2.slug})`);
    linksAdded++;
  }
...
console.log(`Proceso completado. Se han inyectado ${linksAdded} enlaces internos en forma de telaraña semántica.`);
